---
published: true
tags:
 - Artificial Intelligence
 - Machine Learning
 - Papers I Read Recently Series
 - Reading
 - Generative AI
 - Synthetic Data
 - Data Generation
 - Synthetic Data Generation
 - Large Language Models
 - Model Training
 - Model Evaluation
 - LLM as Judge
 - Alignment with Human Feedback
categories:
 - Technology
authors:
 - "Manas Talukdar"
post-format: link
title: Recent AI Reading [07 September 2024]
url-slug: recent-ai-reading-07-september-2024
first-published-on: 2024-09-07 20:06
last-updated-on: 2024-09-07 20:06
series:
 name: Papers I Read Recently
 description: A curated collection of AI/ML papers and research articles I've been reading and found interesting
 part: 5
meta:
 description: "Recent AI reading, including papers and articles."
excerpt: "Best Practices and Lessons Learned on Synthetic Data"
---

# Recent AI Reading [07 September 2024]

## Papers

### Synthetic Data

- [Best Practices and Lessons Learned on Synthetic Data](https://arxiv.org/abs/2404.07503)
- [On LLMs-Driven Synthetic Data Generation, Curation, and Evaluation: A Survey](https://arxiv.org/abs/2406.15126)

### Alignment with Human Feedback / Preferences

- [RLHF Can Speak Many Languages: Unlocking Multilingual Preference Optimization for LLMs](https://arxiv.org/abs/2407.02552)
- [Back to Basics: Revisiting REINFORCE Style Optimization for Learning from Human Feedback in LLMs](https://arxiv.org/abs/2402.14740)
- [A Comprehensive Survey of LLM Alignment Techniques: RLHF, RLAIF, PPO, DPO and More](https://arxiv.org/abs/2407.16216)
- [MAPO: Advancing Multilingual Reasoning through Multilingual Alignment-as-Preference Optimization](https://arxiv.org/abs/2401.06838)
- [Judging the Judges: Evaluating Alignment and Vulnerabilities in LLMs-as-Judges](https://arxiv.org/abs/2406.12624)
- [Direct Preference Optimization: Your Language Model is Secretly a Reward Model](https://arxiv.org/abs/2305.18290)
- [InstructVideo: Instructing Video Diffusion Models with Human Feedback](https://arxiv.org/abs/2312.12490)
- [VideoScore: Building Automatic Metrics to Simulate Fine-grained Human Feedback for Video Generation](https://arxiv.org/abs/2406.15252)
- [RLHF-Blender: A Configurable Interactive Interface for Learning from Diverse Human Feedback](https://arxiv.org/abs/2308.04332)
  - [site](https://sites.google.com/view/rlhfblender)

### Retrieval-Augmented Generation

- [Multi-Meta-RAG: Improving RAG for Multi-Hop Queries using Database Filtering with LLM-Extracted Metadata](https://arxiv.org/abs/2406.13213)
- [HybridRAG: Integrating Knowledge Graphs and Vector Retrieval Augmented Generation for Efficient Information Extraction](https://arxiv.org/abs/2408.04948)
- [EfficientRAG: Efficient Retriever for Multi-Hop Question Answering](https://arxiv.org/abs/2408.04259)

### Other

- [Tree of Thoughts: Deliberate Problem Solving with Large Language Models](https://arxiv.org/abs/2305.10601)
- [Bias-Aware Low-Rank Adaptation: Mitigating Catastrophic Inheritance of Large Language Models](https://arxiv.org/abs/2408.04556v1)
- [How Far Are We From AGI](https://arxiv.org/abs/2405.10313)
- [Consent in Crisis: The Rapid Decline of the AI Data Commons](https://www.dataprovenance.org/consent-in-crisis-paper)
- [Cognitively Inspired Energy-Based World Models](https://arxiv.org/abs/2406.08862)
- [LongWriter: Unleashing 10,000+ Word Generation from Long Context LLMs](https://arxiv.org/abs/2408.07055)
- [The Need for a Leaderboard: A Survey of LLM as a Judge in NLP](https://openreview.net/forum?id=NNFIu4KZpZ)
  - [pdf](https://openreview.net/pdf?id=NNFIu4KZpZ) | [pdf](https://openreview.net/pdf/da2d2999ebc91dd94f4662d49fca433c5d46a6e8.pdf)

## Articles and Blog Posts

- [Prompt caching with Claude](https://www.anthropic.com/news/prompt-caching)
- [Delving into “delve”](https://pshapira.net/2024/03/31/delving-into-delve/)
- [Gorilla: Large Language Model Connected with Massive APIs](https://gorilla.cs.berkeley.edu/blogs/12_bfcl_v2_live.html)
- [Direct Preference Optimization with Synthetic Data on Anyscale](https://www.anyscale.com/blog/direct-preference-optimization-with-synthetic-data)
- [Using LLM-as-a-judge 🧑‍⚖️ for an automated and versatile evaluation](https://huggingface.co/learn/cookbook/en/llm_judge)
- [Databricks announces significant improvements to the built-in LLM judges in Agent Evaluation](https://www.databricks.com/blog/databricks-announces-significant-improvements-built-llm-judges-agent-evaluation)

## Miscellaneous

- [RAG Arena](https://www.ragarena.com/leaderboard)
