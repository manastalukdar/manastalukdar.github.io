---
published: true
tags:
 - Artificial Intelligence
 - Machine Learning
 - Papers I Read Recently Series
 - Reading
 - Generative AI
 - Large Language Models
 - Retrieval-Augmented Generation
 - Model Training
 - Model Evaluation
 - LLM as Judge
 - AI Alignment
 - Agentic AI
 - AI Agents
 - Embodied AI
 - Model Context Protocol
categories:
 - Technology
authors:
 - "Manas Talukdar"
post-format: link
title: Recent AI Reading [25 October 2025]
url-slug: recent-ai-reading-25-october-2025
first-published-on: 2025-10-25 23:38
last-updated-on: 2025-10-25 23:38
series:
 slug: papers-i-read-recently
 part: 15
meta:
 description: "Recent AI reading, including papers and articles."
excerpt: ""
---

# Recent AI Reading [25 October 2025]

## Papers

### Agentic AI

- [Reinforcement Learning for Long-Horizon Interactive LLM Agents](https://arxiv.org/abs/2502.01600)
- [Beyond Ten Turns: Unlocking Long-Horizon Agentic Search with Large-Scale Asynchronous RL](https://arxiv.org/abs/2508.07976)
- [ARE: scaling up agent environments and evaluations](https://blog/2025/10/25/recent-ai-reading-25-oct-2025/readme.mdai.meta.com/research/publications/are-scaling-up-agent-environments-and-evaluations/)
- [Overhearing LLM Agents: A Survey, Taxonomy, and Roadmap](https://arxiv.org/abs/2509.16325)
- [Towards General Agentic Intelligence via Environment Scaling](https://arxiv.org/abs/2509.13311)
- [Democratizing AI scientists using ToolUniverse](https://arxiv.org/abs/2509.23426)
- [Recon-Act: A Self-Evolving Multi-Agent Browser-Use System via Web Reconnaissance, Tool Generation, and Task Execution](https://arxiv.org/abs/2509.21072)
- [Agentic Software Engineering: Foundational Pillars and a Research Roadmap](https://arxiv.org/abs/2509.06216)
- [Scaling Agents via Continual Pre-training](https://arxiv.org/abs/2509.13310)
  - Cross Topics: Training Paradigms
- [EnvX: Agentize Everything with Agentic AI](https://arxiv.org/abs/2509.08088)
- [The Landscape of Agentic Reinforcement Learning for LLMs: A Survey](https://arxiv.org/abs/2509.02547)
  - Cross Topics: Training Paradigms
- [MCP-Bench: Benchmarking Tool-Using LLM Agents with Complex Real-World Tasks via MCP Servers](https://arxiv.org/abs/2508.20453)
- [Memento: Fine-tuning LLM Agents without Fine-tuning LLMs](https://arxiv.org/abs/2508.16153)
  - Cross Topics: Training Paradigms
- [Mobile-Agent-v3: Fundamental Agents for GUI Automation](https://arxiv.org/abs/2508.15144)
- [FutureX: An Advanced Live Benchmark for LLM Agents in Future Prediction](https://arxiv.org/abs/2508.11987)
- [AgentsNet: Coordination and Collaborative Reasoning in Multi-Agent LLMs](https://arxiv.org/abs/2507.08616)
- [A Survey on Agentic Security: Applications, Threats and Defenses](https://arxiv.org/abs/2510.06445)
- [Emergent Coordination in Multi-Agent Language Models](https://arxiv.org/abs/2510.05174)
- [Demystifying Reinforcement Learning in Agentic Reasoning](https://arxiv.org/abs/2510.11701)
  - Cross Topics: Training Paradigms
- [Agentic Context Engineering: Evolving Contexts for Self-Improving Language Models](https://www.arxiv.org/abs/2510.04618)
- [Agent Learning via Early Experience](https://www.arxiv.org/abs/2510.08558)
- [Fundamentals of Building Autonomous LLM Agents](https://arxiv.org/abs/2510.09244)
- [Human-Agent Collaborative Paper-to-Page Crafting for Under $0.1](https://arxiv.org/abs/2510.19600)
- [Thought Communication in Multiagent Collaboration](https://arxiv.org/abs/2510.20733)

### AI Alignment (with Human Preferences, and other methods)

- [Data Shapley in One Training Run](https://arxiv.org/abs/2406.11011)
- [The Evolution of LLM Adoption in Industry Data Curation Practices](https://arxiv.org/abs/2412.16089)
- [GPDVal: Evaluation AI Model Performance on Real-World Economically Valuable Tasks](https://cdn.openai.com/pdf/d5eb7428-c4e9-4a33-bd86-86dd4bcf12ce/GDPval.pdf)
- [The AI Productivity Index (APEX)](https://arxiv.org/abs/2509.25721)
- [HumanAgencyBench: Scalable Evaluation of Human Agency Support in AI Assistants](https://arxiv.org/abs/2509.08494)
- [Behavioral Fingerprinting of Large Language Models](https://arxiv.org/abs/2509.04504)

### Large Language Models

- [The Illusion of Diminishing Returns: Measuring Long Horizon Execution in LLMs](https://www.arxiv.org/abs/2509.09677)
  - [X thread](https://x.com/ShashwatGoel7/status/1966527903568637972).
- [The Illusion of Thinking: Understanding the Strengths and Limitations of Reasoning Models via the Lens of Problem Complexity](https://arxiv.org/abs/2506.06941)
- [LLMs Can Get "Brain Rot"!](https://arxiv.org/abs/2510.13928)
  - [Website](https://llm-brain-rot.github.io)
  - [X thread](https://x.com/hjy836/status/1980061288278421558)
  - [Code](https://github.com/llm-brain-rot/llm-brain-rot)
- [Mathematical research with GPT-5: a Malliavin-Stein experiment](https://www.alphaxiv.org/abs/2509.03065)
- [A Survey on LLM-as-a-Judge](https://arxiv.org/abs/2411.15594)
- [TrustJudge: Inconsistencies of LLM-as-a-Judge and How to Alleviate Them](https://arxiv.org/abs/2509.21117)
- [Mixture of Thoughts: Learning to Aggregate What Experts Think, Not Just What They Say](https://www.arxiv.org/abs/2509.21164)
- [When Does Reasoning Matter? A Controlled Study of Reasoning's Contribution to Model Performance](https://arxiv.org/abs/2509.22193)
- [PersonaX: Multimodal Datasets with LLM-Inferred Behavior Traits](https://arxiv.org/abs/2509.11362)
- [Why Language Models Hallucinate](https://arxiv.org/abs/2509.04664)
- [A Survey of Scientific Large Language Models: From Data Foundations to Agent Frontiers](https://arxiv.org/abs/2508.21148)
  - Cross Topics: Agentic AI
- [Self-Adapting Language Models](https://arxiv.org/abs/2506.10943)
- [InvThink: Towards AI Safety via Inverse Reasoning](https://arxiv.org/abs/2510.01569)

### Training Paradigms

- [Understanding Reinforcement Learning for Model Training, and future directions with GRAPE](https://arxiv.org/abs/2509.04501)
- [Recursive Self-Aggregation Unlocks Deep Thinking in Large Language Models](https://arxiv.org/abs/2509.26626)
  - [Website](https://rsa-llm.github.io)
  - [Code](https://github.com/HyperPotatoNeo/RSA)
- [RL's Razor: Why Online Reinforcement Learning Forgets Less](https://www.arxiv.org/abs/2509.04259)
- [zELO: ELO-inspired Training Method for Rerankers and Embedding Models](https://arxiv.org/abs/2509.12541v1)
- [In Their Own Words: Reasoning Traces Tailored for Small Models Make Them Better Reasoners](https://arxiv.org/abs/2509.22230)
- [Tree Search for LLM Agent Reinforcement Learning](https://www.arxiv.org/abs/2509.21240)
  - Cross Topics: Agentic AI
- [CE-GPPO: Coordinating Entropy via Gradient-Preserving Clipping Policy Optimization in Reinforcement Learning](https://arxiv.org/abs/2509.20712)
- [FlowRL: Matching Reward Distributions for LLM Reasoning](https://arxiv.org/abs/2509.15207)
- [Learning to Optimize Multi-Objective Alignment Through Dynamic Reward Weighting](https://arxiv.org/abs/2509.11452)
  - Cross Topics: AI Alignment
- [A Survey of Reinforcement Learning for Large Reasoning Models](https://arxiv.org/abs/2509.08827)
- [RewardDance: Reward Scaling in Visual Generation](https://arxiv.org/abs/2509.08826)
  - Cross Topics: AI Alignment
- [Pref-GRPO: Pairwise Preference Reward-based GRPO for Stable Text-to-Image Reinforcement Learning](https://arxiv.org/abs/2508.20751)
  - Cross Topics: AI Alignment
- [TreePO: Bridging the Gap of Policy Optimization and Efficacy and Inference Efficiency with Heuristic Tree-based Modeling](https://arxiv.org/abs/2508.17445)
  - Cross Topics: AI Alignment
- [DeepSearch: Overcome the Bottleneck of Reinforcement Learning with Verifiable Rewards via Monte Carlo Tree Search](https://www.arxiv.org/abs/2509.25454)
  - Cross Topics: AI Alignment
- [ExGRPO: Learning to Reason from Experience](https://arxiv.org/abs/2510.02245)
  - Cross Topics: AI Alignment
- [RLFR: Extending Reinforcement Learning for LLMs with Flow Environment](https://arxiv.org/abs/2510.10201)
  - Cross Topics: AI Alignment
- [Verbalized Sampling: How to Mitigate Mode Collapse and Unlock LLM Diversity](https://arxiv.org/abs/2510.01171)
  - Cross Topics: AI Alignment
- [RLP: Reinforcement as a Pretraining Objective](https://arxiv.org/abs/2510.01265)
- [Is In-Context Learning Learning?](https://www.arxiv.org/abs/2509.10414)
- [The Art of Scaling Reinforcement Learning Compute for LLMs](https://www.alphaxiv.org/abs/2510.13786)
- [Every Step Evolves: Scaling Reinforcement Learning for Trillion-Scale Thinking Model](https://arxiv.org/abs/2510.18855)
- [Training-Free Group Relative Policy Optimization](https://arxiv.org/abs/2510.08191)
  - Cross Topics: AI Alignment

### Model Evaluation

- [From Scores to Skills: A Cognitive Diagnosis Framework for Evaluating Financial Large Language Models](https://arxiv.org/abs/2508.13491)

### Retrieval-Augmented Generation

- [End-to-End Agentic RAG System Training for Traceable Diagnostic Reasoning](https://arxiv.org/abs/2508.15746)
  - Cross Topics: Agentic AI, Training Paradigms
- [On-Premise AI for the Newsroom: Evaluating Small Language Models for Investigative Document Search](https://arxiv.org/abs/2509.25494)
  - Cross Topics: Model Evaluation
- [ModernVBERT: Towards Smaller Visual Document Retrievers](https://arxiv.org/abs/2510.01149)

### Embodied AI

- [Embodied AI: From LLMs to World Models](https://arxiv.org/abs/2509.20021)
- [Robotic Control via Embodied Chain-of-Thought Reasoning](https://arxiv.org/abs/2407.08693v2)
- [From reactive to cognitive: brain-inspired spatial intelligence for embodied agents](https://arxiv.org/abs/2508.17198)
  - Cross Topics: Agentic AI

## Books

- [Mathematical Introduction to Deep Learning: Methods, Implementations, and Theory](https://arxiv.org/abs/2310.20360)
- [Understanding Deep Learning](https://udlbook.github.io/udlbook/)

## Technical Reports

- [rStar2-Agent: Agentic Reasoning Technical Report](https://arxiv.org/abs/2508.20722)
  - Cross Topics: Agentic AI

## Articles and Blog Posts

- [Defeating Nondeterminism in LLM Inference - Thinking Machines Lab](https://thinkingmachines.ai/blog/defeating-nondeterminism-in-llm-inference/)
  - Cross Topics: Large Language Models
- [Improving Cursor Tab with online RL](https://cursor.com/en-US/blog/tab-rl)
  - Cross Topics: Training Paradigms
- [Building LangGraph: Designing an Agent Runtime from first principles](https://blog.langchain.com/building-langgraph/)
  - Cross Topics: Agentic AI
- [A Definition of AGI](https://www.agidefinition.ai/?s=09)
  - [X Post](https://x.com/DanHendrycks/status/1978828377269117007)
  - [Paper](https://www.agidefinition.ai/paper.pdf)

## Miscellaneous

- [Statistical Methods in Generative AI](https://arxiv.org/abs/2509.07054)
- [Visual Story-Writing: Writing by Manipulating Visual Representations of Stories](https://arxiv.org/abs/2410.07486)
  - 🎥Video: <https://youtu.be/6o19_D8LBC0>
  - 🖥️Code: <https://github.com/m-damien/VisualStoryWriting>
  - 🕹️Demo: <https://damienmasson.com/VisualStoryWriting/>
  - X post: <https://x.com/damienhci/status/1963246088674017478>
- [Build Reliable Agents With RL](https://openpipe.ai/?refresh=1756955210542)
  - [Code for Agent Reinforcement Trainer](https://github.com/OpenPipe/ART)
- [Executive Concerns with AI Adoption: Identifying Business and Security Risks](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=5405290)
