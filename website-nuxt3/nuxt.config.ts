...
const siteDescription =
  'Manas Talukdar is a software engineering manager in large-scale distributed systems, experienced in growing organizations and running high performing teams.'
...
export default defineNuxtConfig({
  ...
  build: {
    ...
    extend(config, ctx) {
      ...
      const feed = new Feed({
        id: "manastalukdar-personal-website",
        title: siteOwner + ' - blog',
  ...
  redirect: [
    {
      ...
      from: '^.*(?<!/)$', // ^.*(?<!\.(png|jpg))$    ^.*(?<!/)$
      to: (from: any, req: { url: string }) => req.url + '/',
    },
  ],
